...
def plot_single_distribution(predictions, file_name, save_dir):
    ...
    if non_zero_categories:
        ax3.pie(non_zero_categories, labels=non_zero_labels, colors=non_zero_colors,
...
def create_summary_table(all_stats, save_dir):
    ...
    # 创建可视化表格
    fig, ax = plt.subplots(figsize=(16, max(6, len(sorted_stats) * 0.5)))
    ax.axis('tight')
    ax.axis('off')
...
def main():
    ...
    if not submission_files:
        print("\n❌ 没有找到submission文件!")
        print("请先运行train_with_gan.py生成预测结果")
        return

    print(f"\n找到 {len(submission_files)} 个submission文件:")
    ...
    if all_stats:
        ...
        print(f"平均概率最高: {sorted_by_mean[0]['file_name']} ({sorted_by_mean[0]['mean']:.4f})")
        print(f"平均概率最低: {sorted_by_mean[-1]['file_name']} ({sorted_by_mean[-1]['mean']:.4f})")

        sorted_by_std = sorted(all_stats, key=lambda x: x['std'])
        print(f"预测最稳定: {sorted_by_std[0]['file_name']} (标准差={sorted_by_std[0]['std']:.4f})")
        print(f"预测最不稳定: {sorted_by_std[-1]['file_name']} (标准差={sorted_by_std[-1]['std']:.4f})")
    ...
    print(f"  - 汇总CSV: predictions_summary.csv")
    print(f"  - 汇总表格图: predictions_summary_table.png")
    print()
